  ...
  for (s, d), (t, f) in delta_F.items():
    ...
    with torch.no_grad():
      ...
      if s in H and d in H:
        source_ids = torch.from_numpy(np.array([s], dtype = int)).to(device)
        target_ids = torch.from_numpy(np.array([d], dtype = int)).to(device)
      ...
      for (s_, d_) in Xi_out[s]:
        if s_ in H and d_ in H:
          source_ids = torch.from_numpy(np.array([s_], dtype = int)).to(device)
          target_ids = torch.from_numpy(np.array([d_], dtype = int)).to(device)
          lambda_Xi_out_s += model(source_ids, target_ids).item()
        else:
          lambda_Xi_out_s += f_th
      if lambda_Xi_out_s == 0:
        continue
  ...
  if flag == "global":
    ...
    for epoch in range(1, epochs + 1):
      ...
      while batch_s < train_num:
        ...
        batch_source_ids_np = np.array([x[0] for x in batch], dtype = int)
        batch_source_ids = torch.from_numpy(batch_source_ids_np).to(device = device)
        batch_target_ids_np = np.array([x[1] for x in batch], dtype = int)
    ...
    for epoch in range(1, epochs + 1):
      ...
      batch_source_ids_np = np.array([x[0] for x in train_data], dtype = int)
      batch_source_ids = torch.from_numpy(batch_source_ids_np).to(device = device)
      batch_target_ids_np = np.array([x[1] for x in train_data], dtype = int)
      batch_target_ids = torch.from_numpy(batch_target_ids_np).to(device = device)
...
def F_FADE(model, dataset, t_setup, W_upd, alpha, M, T_th, epochs, online_train_steps, max_batch_size, device):
  ...
  # Step 2
  # for t in tqdm(range(1, T_max + 1), desc="Time steps"):
  for t in tqdm(range(0, T_max + 1), desc="Time steps"):
    # Step 3
    delta_F = dict()
