...
def train_model_link_pred(node_feats, edge_feats, g, df, model, mailbox, sampler, sample_param, memory_param, gnn_param, train_param, args, seed=0):
    ...
    train_df = df[df['ext_roll'] == 0]   # defining train samples
    ...
    for e in range(1, train_param['epoch'] + 1):
        ...
        for _, rows in train_df.groupby(train_df.index // train_param['batch_size']):
            ...
            mfgs = prepare_input(mfgs, node_feats, edge_feats, combine_first=combine_first)  # this is done for each epoch
            if mailbox is not None:
            ...
            # we can check shape of mfgs in each epoch
            optimizer.zero_grad()
            if args.robust == "none":
                # print("I came here 3")
        ...
        # Do we need to save the model in each epoch?
        model_path = f'{scratch_location}/tspear/MODEL/{args.exp_id}/{args.exp_file}_seed_{seed}_{e}.pt'
        if args.attack == "none":
            print(f'saving model at {model_path}', end='\r')
            torch.save(model.state_dict(), model_path)
            print(f'saved model at {model_path}.   ', end='\r')
        ...
        # args.logger.debug(f'Epoch: {e}, train_loss: {total_loss:.1f}, val_ap: {val_ap:.4f}, val_auc: {val_auc:.4f}, test_ap: {test_ap:.4f}, test_auc: {test_auc:.4f} total_time: {(time_tot + time_val):.2f}s (sample: {time_sample:.2f}s, prep: {time_prep:.2f}s, val: {time_val:.2f}s)')
        ...
        # args.logger.debug(f'Epoch: {e}, train_loss: {total_loss:.1f}, test_ap: {test_ap:.4f}, test_auc: {test_auc:.4f}')
...
def robust_train_model_link_pred_v2(node_feats, edge_feats, g, df, model, mailbox, sampler, sample_param, memory_param, gnn_param, train_param, args, seed=0):
    ...
    for e in range(1, train_param['epoch'] + 1):
        ...
        # clean_g = copy.deepcopy(g)  ## causes error: TypeError: cannot pickle '_io.BufferedReader' object
        clean_g = copy.copy(g)  
        clean_df = df.copy()
        ...
        for i_rows, rows in train_df.groupby(train_df.index // train_param['batch_size']):
            ...
            if args.robust == "proposed":
                ...
                n_adv = n_adv + remove_df['adv'].eq(1).sum()
        ...
        print("% of adversarial samples:", n_adv / num_edge_filter * 100)
        final_avg = final_avg + n_adv / num_edge_filter * 100 if num_edge_filter > 0 else final_avg
    ...
    model.eval()
    final_avg = final_avg / train_param['epoch']
    print("Final Average of adversarial samples:",final_avg)
    # val_ap, val_auc, val_hit, val_g, val_df = robust_link_pred_evaluation_v2(node_feats, edge_feats, clean_g, clean_df, model, mailbox, sampler, sample_param, memory_param, gnn_param, train_param, args, negs=1, mode='val', evaluation=False)
    ...
    if args.robust == "cosine" or args.robust == "proposed":
        return val_ap, val_auc, val_hit, test_ap, test_auc, test_hit, final_avg
    else:
        return val_ap, val_auc, val_hit, test_ap, test_auc, test_hit
...
def remove_noise(g, df, remove_df):
    ...
    for i, row in remove_df.iterrows():
        ...
        for node in [src, dst]:
            ...
            # print("\tg['eid']:", g['eid'][idx_start:idx_end])
            # print("\teid:", eid)
            ...
            if idx_end == idx_start:
                ...
                matches = np.where(g['eid'][idx_start:idx_end] == eid)[0]
                if matches.size > 0:
                    idx_rmv = idx_start + matches[0]
    ...
    mask = np.ones_like(g['eid']).astype(bool)
    if len(idx_rmvs)>0:
        mask[idx_rmvs] = False
    # mask[idx_rmvs] = False
    # g = dict(g)
...
def __remove_noise(g, df, remove_df):
    print(type(g), end=' --> ')
    # Convert the read-only NpzFile to a mutable dictionary
    g = {key: g[key].copy() for key in g.keys()}
    print(type(g))
...
def link_pred_evaluation(node_feats, edge_feats, g, df, model, mailbox, sampler, sample_param, memory_param, gnn_param, train_param, args, negs=1, mode='val', seed=None, evaluation=False):
    ...
    with torch.no_grad():
        for _, rows in eval_df.groupby(eval_df.index // train_param['batch_size']):
            ...
            # # Print the cached memory (used by PyTorch's memory allocator)
            # cached_memory = torch.cuda.memory_reserved(0)
            # print(f"Memory Cached: {cached_memory / (1024 ** 2)} MB")
            # time.sleep(0.1)
            if evaluation and args.data == "MOOC": 
            ...
            y_pred = torch.cat([pred_pos, pred_neg], dim=0).squeeze().sigmoid().cpu().detach()
            y_true = torch.cat([torch.ones(pred_pos.size(0)), torch.zeros(pred_neg.size(0))], dim=0).cpu().detach()
            pred_pos.cpu()
            pred_neg.cpu()
            torch.cuda.empty_cache()
            ...
            if args.attack != "none" and mode == "test":
                # print('\ttesting for non adv...')
                # print('\trows:', len(rows), rows.columns)
                # print(rows['adv'])
                # exit()
                pos_mask = (rows['adv'] == 0).values
                # print(pos_mask)
                # print(f'\t\t{pos_mask.sum()} / {len(pos_mask)}')
                neg_mask = np.tile(pos_mask, neg_samples)
                # print(f'\t\t{neg_mask.sum()} / {len(neg_mask)}')
                pos_neg_mask = np.concatenate((pos_mask, neg_mask))
                # print(f'\t\t{pos_neg_mask.sum()} / {len(pos_neg_mask)}')
                # print(f'\t\t{len(y_true) = }')
                # print(f'\t\t{len(y_pred) = }')
