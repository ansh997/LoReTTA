...
def parse_argument():
    ...
    parser.add_argument("--surrogate", type=str, default="TGN")
    ...
    parser.add_argument("--use_clean_eval", action="store_true", default=False)
    parser.add_argument("--use_clean_test", action="store_true", default=False)
...
def get_argument():
    ...
    parser.add_argument("--attack", type=str, default="none", choices=["none", "random", "proposed", "degree", "pagerank", "preference", "jaccard", 'ss_ns'])
    parser.add_argument("--surrogate", type=str, default="TGN")
    parser.add_argument("--ptb_rate", type=float, default=0.3)
    parser.add_argument("--use_hungarian", action="store_true", default=True)
    parser.add_argument("--xpool", type=int, default=0)
    ...
    parser.add_argument('--ss', type=str, help='sparsification strategy name', default='random')
    parser.add_argument('--upto', type=str, help='sparsification upto', default='0.7')
    parser.add_argument('--id', type=str, help='experiment name', default=None)
    parser.add_argument('--eval_neg_samples', type=int, default=1, help='how many negative samples to use at inference. Note: this will change the metric of test set to AP+AUC to AP+MRR!')
    parser.add_argument('--sprs_only', action='store_true', default=False, help='use only sparsified data')
    args = parser.parse_args()
    os.environ['CUDA_VISIBLE_DEVICES'] = str(args.gpu)
    return args
...
def pre_exp_setting(exp_id, args, evaluation=False):
    ...
    if args.robust == "proposed":
        ...
        exp_file += f'_{args.scheduler}_{args.threshold_s}_{args.threshold_e}_{args.temporal}_{args.theta}'
    elif args.robust == "cosine":
        exp_file += f'_{args.scheduler}_{args.cosine_threshold}'
    elif args.robust == "svd":
        # exp_file += f'_{args.svd_rank}_{args.svd_threshold}_{args.mf_window}'
        exp_file += f'_{args.svd_rank}'
    ...
    print("Log Location")
    print(f'{scratch_location}/tspear/LOG/{exp_id}/{exp_dir}/')
...
def load_data(data, args):
    ...
    if data in ["MOOC", "LASTFM", "UCI", "BITCOIN", "ENRON", "SOCIALEVO", "BITCOIN2"]:
        rand_edge_features = 128
        rand_node_features = 128
    node_feats, edge_feats = load_feat(data, rand_edge_features, rand_node_features)  # same as TGL main
    # node_feats, edge_feats = None, None
    g, df = load_graph(data) # same as TGL main
    # if data == "UCI":
...
def load_graph(d):
    ...
    # df.rename(columns={'u': 'src', 'i': 'dst', 'ts': 'time'}, inplace=True)  # if present
    # try:
    #     g = np.load('{}/tspear/DATA/{}/ext_full.npz'.format(scratch_location, d))
...
def get_attacked_data_dir(args):
    if args.attack == "proposed":
        ...
        data_dir = f'{scratch_location}/tspear/DATA/{args.data}/ATTACKED/{args.attack}_{args.ptb_rate}'
        # data_dir = f'{scratch_location}/sparsified_data/{args.data}/{args.attack}'
    ...
    if node_feats is not None:
        for b in mfgs[0]:
            if pinned:
                ...
                # print(f"node_feats is on: {node_feats.device}")
                # print(f"b.srcdata['ID'] is on: {b.srcdata['ID'].device}")
                src_ids = b.srcdata['ID'].to(node_feats.device)
                # srch = node_feats[b.srcdata['ID'].long()].float()
                srch = node_feats[src_ids.long()].float()
...
def load_poisoned_graph(d, nss, ss, upto, flag=False, sprs_only=False):
    if flag:
        save_location = f'{scratch_location}/poisoned_data_icml/nss_{nss}/{d}/{ss}'
    else:
        # save_location = f'{scratch_location}/poisoned_data_icml/poisoned_data/{d}/{ss}'  # checking against ICML
        save_location = f'{scratch_location}/poisoned_data_AAAI/{d}/{ss}'
    if not sprs_only:
        df = pd.read_csv(f'{save_location}/modified_{d}_{ss}_sparsified_{upto}.csv')
    else:
        df = pd.read_csv(f'{save_location}/modified_sprs_only_{d}_{ss}_sparsified_{upto}.csv')
    
    if ss!='untouched':
        if not sprs_only:
            # g = np.load(f'{save_location}/modified_{d}_{ss}_sparsified_{upto}_ext_full.npz', allow_pickle=True) # maybe for each poisoned dataset we produce a ext_full.npz
            print('loading pickle file')
            with open(f'{save_location}/ext_full.pkl', 'rb') as f:
                g = pickle.load(f)
        else:
            g = np.load(f'{save_location}/modified_sprs_only_{d}_{ss}_sparsified_{upto}_ext_full.npz') # maybe for each poisoned dataset we produce a ext_full.npz
            
    else:
        g = np.load(f'{save_location}/ext_full.npz') # maybe for each poisoned dataset we produce a ext_full.npz
    
    
    # we should load pkl file
    ...
    # with open(f'{scratch_location}/tspear/DATA/{d.upper()}/ext_full.pkl', 'rb') as f:
    #     g = pickle.load(f)
    g = dict(g) # added by pranav 
    return g, df
...
def load_poisoned_feat(d, rand_de=0, rand_dn=0, ss='ts_tpr_remove_mss', upto=0.7):
    ...
    if os.path.exists('/raid/t2/TGN_adv/poisoned_data_AAAI/{}/{}/node_features.pt'.format(d, ss, ss, upto)):
        node_feats = torch.load('/raid/t2/TGN_adv/poisoned_data_AAAI/{}/{}/node_features.pt'.format(d, ss, ss, upto))
        if node_feats.dtype == torch.bool:
            node_feats = node_feats.type(torch.float32)
    edge_feats = None
    if os.path.exists('/raid/t2/TGN_adv/poisoned_data_AAAI/{}/{}/edge_features.pt'.format(d, ss, ss, upto)):
        print('\tLoading edge features.')
        edge_feats = torch.load('/raid/t2/TGN_adv/poisoned_data_AAAI/{}/{}/edge_features.pt'.format(d, ss, ss, upto))
        if edge_feats.dtype == torch.bool:
            edge_feats = edge_feats.type(torch.float32)
    if rand_de > 0:
        if d.upper() == 'LASTFM':
            ...
            edge_feats = torch.randn(int(59835*1.3), rand_de)
        elif d.upper() == "BITCOIN":
            edge_feats = torch.randn(35590, rand_de)
        elif d.upper() == "ENRON":
            edge_feats = torch.randn(int(125236*1.3), rand_de)
        elif d.upper() == "WIKIPEDIA":
            edge_feats = torch.randn(int(157475*1.3), rand_de)
    if rand_dn > 0:
        if d.upper() == 'LASTFM':
            node_feats = torch.randn(1980, rand_dn)
        elif d.upper() == 'MOOC':
            ...
            node_feats = torch.randn(int(9228), rand_de)
    return node_feats, edge_feats


def load_poisoned_data(data, args, sprs_only=False):
    ss, upto= args.ss, args.upto
    rand_edge_features = 0 if not hasattr(args, 'rand_edge_features') else args.rand_edge_features
    rand_node_features = 0 if not hasattr(args, 'rand_node_features') else args.rand_node_features
    if data.upper() in ["MOOC", "LASTFM", "UCI", "BITCOIN", "ENRON", "SOCIALEVO", "WIKIPEDIA"]:
        rand_edge_features = 128
        rand_node_features = 128
    node_feats, edge_feats = load_poisoned_feat(data, rand_edge_features, rand_node_features, ss=ss, upto=upto)
    # node_feats, edge_feats = None, None 
    g, df = load_poisoned_graph(data, nss='', ss=ss, upto=upto, sprs_only=sprs_only)
    # if data == "UCI":
    #     edge_feats = torch.randn(len(df), 128)
    return node_feats, edge_feats, g, df
