...
class TemporalAttack:
    ...
    def attack(self, orig_node_feats, orig_edge_feats, orig_g, orig_df, ptb_rate, args, seed=0, **kwargs):
        ...
        tot_num_ptb = 0
        print('Running Training data only...')
        for i_df, x_df in enumerate([train_df]):
            ...
            for i_rows, rows in x_df.groupby(x_df.index // args.batch_size):
                if i_rows > 0:
                    ...
                    if args.attack == "random":
                        ...
                        ptb_src_batch, ptb_dst_batch = [], []
                        
                        prev_i = 0
                        for i in row_src_array[row_cand[idx_batch]]:
                            # print(f'{i = }')
                            ptb_src_batch.append(idx_to_src.get(i, idx_to_src[prev_i]))
                            if i not in idx_to_src.keys():
                                continue
                            else:
                                prev_i = i
                        
                        prev_j = 0
                        for j in row_dst_array[col_cand[idx_batch]]:
                            # print(f'{j = }', end='\r')
                            ptb_dst_batch.append(idx_to_dst.get(j, idx_to_dst[prev_j])) 
                            if j not in idx_to_dst.keys():
                                continue
                            else:
                                prev_j = j
                            
                        
                        ptb_src_batch = np.array(ptb_src_batch).astype(np.int32)
                        ptb_dst_batch = np.array(ptb_dst_batch).astype(np.int32)
