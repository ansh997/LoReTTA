...
def train_main(args):
    ...
    for i, seed in enumerate(args.seeds):
        ...
        if args.attack == "none":
            node_feats, edge_feats, g, df = load_poisoned_data(args.data, args, sprs_only=args.sprs_only)
        else:
            # node_feats, edge_feats, g, df = load_attacked_data(args.data, args)
            raise NotImplementedError('Not intended to be used.')
    ...
    row_data = {
        **vars(args),
        ...
        "Final Avg": f"{final_avg}"
        }
    
    df = pd.DataFrame([row_data])
    current_time = datetime.now().strftime('%Y%m%d')
    args.id = current_time if args.id is None else args.id
    output_file = f"Our_Attacks_results_neg_samples{args.eval_neg_samples}_{args.id}.csv"
