...
def append_row_to_csv(filename, dataset, config, AP, MRR):
    # Define the fieldnames (column headers)
    # fieldnames = ['dataset', 'nss', 'ss', 'upto', 'AP', 'MRR']
    fieldnames = ['dataset', 'config', 'AP', 'MRR']
...
if args.use_inductive:
    test_df = df[val_edge_end:]
    # inductive_nodes = set(test_df.src.values).union(test_df.src.values)  # original
    inductive_nodes = set(test_df.src.values).union(test_df.dst.values)
    print("inductive nodes", len(inductive_nodes))
    # neg_link_sampler = NegLinkInductiveSampler(inductive_nodes)  # original
    neg_link_sampler = NegLinkSamplerDST(inductive_nodes)
else:
    # neg_link_sampler = NegLinkSampler(g['indptr'].shape[0] - 1)  # original'
    test_df = df[val_edge_end:]
    # inductive_nodes = set(test_df.src.values).union(test_df.src.values)  # original
    inductive_nodes = set(test_df.src.values).union(test_df.dst.values)
    print("inductive nodes", len(inductive_nodes))
...
filename = f'Baseline_{args.data}_results_neg_samples_{args.eval_neg_samples}_{current_time}.csv'
if args.eval_neg_samples > 1:
    print('\ttest AP:{:4f}  test MRR:{:4f}'.format(ap, auc))
    append_row_to_csv(filename, args.data, args.config, ap, auc)
else:
