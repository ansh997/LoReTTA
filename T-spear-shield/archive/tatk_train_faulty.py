...
def train_main(args):
    ...
    for i, seed in enumerate(args.seeds):
        set_random_seed(seed)  # sets seed for every run

        if args.attack == "none":
            node_feats, edge_feats, g, df = load_data(args.data, args)  # loads dataset same as TGL
        else:
            node_feats, edge_feats, g, df = load_attacked_data(args.data, args)  # loads dataset same as TGL
        
        ## Model train
        sample_param, memory_param, gnn_param, train_param = parse_config(args.model)  # loads dataset same as TGL
        
        # encapuslates all of TGL code into a util func
