...
def _add_negative_samples(sprs_df, orig_df, full_graph):
    ...
    all_early_missing_edges_df = find_missing_edges(orig_df, sprs_df, start_time=early_start_time, end_time=early_end_time, batch_size=2)
...
def get_poisoned_data(dataset_name='wikipedia', strategy='random', upto=0.7, val_ratio=0.15, test_ratio=0.15, 
    ...
    os.makedirs(f'{save_location}/poisoned_data2/{dataset_name}/{strategy}', exist_ok=True)
    
    OUT_DF = f'{save_location}/poisoned_data2/{dataset_name}/{strategy}/{dataset_name}_{strategy}_sparsified_{upto}.csv'
    OUT_FEAT = '{}/poisoned_data2/{}/{}/ml_{}.npy'.format(save_location, dataset_name, strategy, dataset_name)
    OUT_NODE_FEAT = '{}/poisoned_data2/{}/{}/ml_{}_node.npy'.format(save_location, dataset_name, strategy, dataset_name)
...
def process_batches_sequentially(missing_edges_df, full_graph,train_graph ,max_workers=None, is_bipartite=True, sequential=False):
    """Process all batches in parallel"""
    ...
    def _process_single_batch(missing_edges_df, full_graph, idx, already_added, is_bipartite=True):
        ...
        # Process edges and timestamps
        df_sorted = train_graph.sort_values('ts')
        original_edges = set(tuple(x) for x in df_sorted[['u', 'i']].values)
...
if __name__ == '__main__':
    ...
    if args.dataset_name in []:  # DONE: remove UCI from here
        Path("{}/poisoned_data2/{}/".format(save_location, args.dataset_name)).mkdir(parents=True, exist_ok=True)
        copy_tree("{}/DG_data/{}/".format(scratch_location, args.dataset_name), "{}/poisoned_data2/{}/".format(save_location, args.dataset_name))
        print(f'Not implemented for enron, SocialEvo graph yet.')
    else:
        Path("{}/poisoned_data2/{}/".format(scratch_location, args.dataset_name)).mkdir(parents=True, exist_ok=True)
        copy_tree("{}/DG_data/{}/".format(scratch_location, args.dataset_name), "{}/poisoned_data2/{}/".format(save_location, args.dataset_name))
        # bipartite dataset
