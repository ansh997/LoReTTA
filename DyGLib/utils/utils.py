...
class AdversarialEdgeSampler(object):
    def __init__(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray, interact_times: np.ndarray = None,
                 last_observed_time: float = None, negative_sample_strategy: str = 'random', 
                 seed: int = None, time_window: float = 1200):
        self.seed = seed
        self.negative_sample_strategy = negative_sample_strategy
        self.src_node_ids = src_node_ids
        self.dst_node_ids = dst_node_ids
        ...
        self.node_last_active_time = defaultdict(float)
        self._update_node_active_times(src_node_ids, dst_node_ids, interact_times)
    ...
    def _update_node_active_times(self, src_node_ids, dst_node_ids, interact_times):
        for src, dst, time in zip(src_node_ids, dst_node_ids, interact_times):
            self.node_last_active_time[src] = max(self.node_last_active_time[src], time)
            self.node_last_active_time[dst] = max(self.node_last_active_time[dst], time)

    def get_active_nodes(self, current_time: float):
        """
        Get nodes that were active within the time window before the current time.
        """
        active_nodes = [node for node, last_active in self.node_last_active_time.items()
                        if current_time - last_active <= self.time_window]
    ...
    def sample(self, size: int, batch_src_node_ids: np.ndarray = None, batch_dst_node_ids: np.ndarray = None,
               current_batch_start_time: float = 0.0, current_batch_end_time: float = 0.0):
        self._update_node_active_times(batch_src_node_ids, batch_dst_node_ids, 
                                       np.full_like(batch_src_node_ids, current_batch_end_time))

        if self.negative_sample_strategy == 'random':
            negative_src_node_ids, negative_dst_node_ids = self.random_sample(size=size, current_time=current_batch_end_time)
        elif self.negative_sample_strategy == 'historical':
    ...
    def random_sample(self, size: int, current_time: float):
        ...
        if self.seed is None:
            src_indices = np.random.randint(0, len(active_nodes), size)
            dst_indices = np.random.randint(0, len(active_nodes), size)
        else:
            src_indices = self.random_state.randint(0, len(active_nodes), size)
            dst_indices = self.random_state.randint(0, len(active_nodes), size)
        
        # Ensure src and dst are different
        same_indices = src_indices == dst_indices
        while np.any(same_indices):
            if self.seed is None:
                dst_indices[same_indices] = np.random.randint(0, len(active_nodes), np.sum(same_indices))
            else:
                dst_indices[same_indices] = self.random_state.randint(0, len(active_nodes), np.sum(same_indices))
            same_indices = src_indices == dst_indices

        return active_nodes[src_indices], active_nodes[dst_indices]

    def historical_sample(self, size: int, batch_src_node_ids: np.ndarray, batch_dst_node_ids: np.ndarray,
                          current_batch_start_time: float, current_batch_end_time: float):
        # Implement historical sampling considering the time window constraint
        # This is a placeholder and needs to be implemented
        raise NotImplementedError("Not yet implemented.")

    def inductive_sample(self, size: int, batch_src_node_ids: np.ndarray, batch_dst_node_ids: np.ndarray,
                         current_batch_start_time: float, current_batch_end_time: float):
        # Implement inductive sampling considering the time window constraint
        # This is a placeholder and needs to be implemented
        raise NotImplementedError("Not yet implemented.")
