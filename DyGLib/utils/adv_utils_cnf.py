...
class AdvNegativeEdgeSampler(object):
    ...
    def __init__(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray, interact_times: np.ndarray = None, last_observed_time: float = None,
        ...
        print(f'negative sampling strategy is {self.negative_sample_strategy}.')
    ...
    def sample(self, size: int, batch_src_node_ids: np.ndarray = None, batch_dst_node_ids: np.ndarray = None,
        ...
        :return:
        """
        if len(batch_src_node_ids) == 1:
            fixed_src_node = batch_src_node_ids[0]
        else:
            raise ValueError('Please provide a fixed source nodes to generate adversarial negative samples')
        if self.negative_sample_strategy == 'random':
            negative_src_node_ids, negative_dst_node_ids = self.random_sample(size=size, fixed_src_node=fixed_src_node)
    ...
    def random_sample_(self, size: int):
        """
    ...
    def random_sample(self, size: int, fixed_src_node: int = None):
        """
        Random sampling strategy with option to fix the source node.
        :param size: int, number of sampled negative edges
        :param fixed_src_node: int, the source node to use for all samples (if provided)
        :return: tuple of arrays (src_nodes, dst_nodes)
        """
        if fixed_src_node is not None:
            # Use the fixed source node for all samples
            src_nodes = np.full(size, fixed_src_node)
        else:
            # Sample random source nodes as before
            if self.seed is None:
                src_indices = np.random.randint(0, len(self.unique_src_node_ids), size)
            else:
                src_indices = self.random_state.randint(0, len(self.unique_src_node_ids), size)
            src_nodes = self.unique_src_node_ids[src_indices]

        # Sample destination nodes
        if self.seed is None:
            dst_indices = np.random.randint(0, len(self.unique_dst_node_ids), size)
        else:
            dst_indices = self.random_state.randint(0, len(self.unique_dst_node_ids), size)
        dst_nodes = self.unique_dst_node_ids[dst_indices]

        # Ensure that src and dst are not the same
        same_node_mask = src_nodes == dst_nodes
        while np.any(same_node_mask):
            if self.seed is None:
                new_dst_indices = np.random.randint(0, len(self.unique_dst_node_ids), np.sum(same_node_mask))
            else:
                new_dst_indices = self.random_state.randint(0, len(self.unique_dst_node_ids), np.sum(same_node_mask))
            dst_nodes[same_node_mask] = self.unique_dst_node_ids[new_dst_indices]
            same_node_mask = src_nodes == dst_nodes
    ...
    def historical_sample(self, size: int, batch_src_node_ids: np.ndarray, batch_dst_node_ids: np.ndarray,
        ...
        print(f'In Historical sampling - {self.negative_sample_strategy=}')

        # Get all historical edges up to current_batch_start_time
        historical_mask = self.interact_times < current_batch_start_time
        historical_src = self.src_node_ids[historical_mask]
        historical_dst = self.dst_node_ids[historical_mask]

        # Create a dictionary of historical destinations for each source
        historical_dst_dict = defaultdict(set)
        for src, dst in zip(historical_src, historical_dst):
            historical_dst_dict[src].add(dst)

        # Create a set of nodes active before current_batch_start_time
        active_nodes = set(historical_src) | set(historical_dst)

        negative_src_node_ids = []
        negative_dst_node_ids = []

        # Process each unique source node in the batch
        unique_batch_src = np.unique(batch_src_node_ids)
        for src in unique_batch_src:
            if src not in active_nodes:
                continue

            # Get all possible destination nodes for this source
            possible_dst = [
                dst for dst in active_nodes
                if dst not in historical_dst_dict[src] and
                dst not in batch_dst_node_ids[batch_src_node_ids == src] and
                dst != src
            ]
            ...
            # Determine number of samples for this source node
            src_count = np.sum(batch_src_node_ids == src)
            src_sample_size = min(size * src_count, len(possible_dst))

            # Sample destinations for this source
            sampled_dst = self.random_state.choice(possible_dst, size=src_sample_size, replace=False)

            negative_src_node_ids.extend([src] * src_sample_size)
            negative_dst_node_ids.extend(sampled_dst)

        # Assign timestamps to negative samples
        negative_timestamps = np.random.uniform(current_batch_start_time, current_batch_end_time, len(negative_src_node_ids))  #maybe later

        return np.array(negative_src_node_ids, dtype=np.longlong), np.array(negative_dst_node_ids, dtype=np.longlong)
    ...
    def inductive_sample(self, size: int, batch_src_node_ids: np.ndarray, batch_dst_node_ids: np.ndarray,
        ...
        historical_mask = (self.interact_times >= self.earliest_time) & (self.interact_times < current_batch_start_time)
        historical_src = self.src_node_ids[historical_mask]
        historical_dst = self.dst_node_ids[historical_mask]
        ...
        unique_batch_src = np.unique(batch_src_node_ids)
        for src in unique_batch_src:
            # Get all possible inductive destination nodes for this source
            # print(type(historical_dst_dict), type(self.observed_edges), type(batch_dst_node_ids))
            # possible_dst = set(historical_dst_dict[src]) - set(self.observed_edges[src]) - set(batch_dst_node_ids[batch_src_node_ids == src])
            possible_dst = list(set(self.unique_dst_node_ids) - historical_dst_dict[src] - set(batch_dst_node_ids[batch_src_node_ids == src]))
            possible_dst = list(possible_dst)
        ...
        if len(negative_src_node_ids) < size * len(batch_src_node_ids):
            remaining_size = size * len(batch_src_node_ids) - len(negative_src_node_ids)
            random_src, random_dst = self.random_sample_with_collision_check(
                size=remaining_size,
                batch_src_node_ids=batch_src_node_ids,
                batch_dst_node_ids=batch_dst_node_ids
            )
            negative_src_node_ids.extend(random_src)
            negative_dst_node_ids.extend(random_dst)
