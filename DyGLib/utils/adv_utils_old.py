...
class AdvNegativeEdgeSampler(object):
    ...
    def random_sample(self, size: int, batch_src_node_ids):
        ...
        print('\tbatch_src_node_ids', batch_src_node_ids)
        
        if self.seed is None:
            # random_sample_edge_src_node_indices = np.random.randint(0, len(self.unique_src_node_ids), size)
            random_sample_edge_src_node_indices = np.random.choice(batch_src_node_ids, size, replace=True)
        ...
        try:
            (self.unique_src_node_ids[random_sample_edge_src_node_indices], self.unique_dst_node_ids[random_sample_edge_dst_node_indices])
        except Exception as _:
            # print(f'{random_sample_edge_src_node_indices=} {random_sample_edge_dst_node_indices}')
            print(len(self.unique_dst_node_ids), f'{random_sample_edge_dst_node_indices=}')
            print(len(self.unique_src_node_ids), f'{random_sample_edge_src_node_indices=}')
            assert 0 == 1, f'Broken for {batch_src_node_ids[0]}'
        print(f'{len(random_sample_edge_src_node_indices) = } {self.unique_src_node_ids[random_sample_edge_src_node_indices] = }')
