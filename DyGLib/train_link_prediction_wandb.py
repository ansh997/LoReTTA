...
from utils.load_configs_wandb import get_link_prediction_args_wandb

if __name__ == "__main__":

    warnings.filterwarnings('ignore')
    # Set the logging level for the urllib3 library
    logging.getLogger("urllib3").setLevel(logging.WARNING)

    # Optional: Set the logging level for the wandb library
    logging.getLogger("wandb").setLevel(logging.WARNING)

    # get arguments
    args = get_link_prediction_args_wandb(is_evaluation=False)
    
    # ==============================
    # Hyperparameter Sweep Configuration
    # ==============================
    sweep_configuration = {'method': 'bayes', # change method here too
                        'name': f'{args.dataset_name}_{args.sampling_upto}_{args.optimizer}',
                        'metric': {'goal': 'maximize', 
                                    'name': f'test mrr_{args.dataset_name}_{args.sampling_upto}'},                              
                        'parameters':{'lr': {'max': args.lr_max, 'min': args.lr_min},
                                    # 'sampling_upto': {'max': args.max_sampling_upto, 'min': args.min_sampling_upto},                               
                                    }
                        }

    sweep_id = wandb.sweep(sweep = sweep_configuration, project = 'AdvAttack_Link_Pred')
    ...
    def main(args=args):
        ...
        wandb.init(name = args.dataset_name + '-' + str(args.sampling_upto))
        
        # =================
        # Hyperparam Set
        # =================
        lr = wandb.config.lr
        # sampling_upto = wandb.config.sampling_upto
        # print(f'{lr=}, {sampling_upto=}')
        # =================
        ...
        for run in range(args.num_runs):
            ...
            args.seed = run
            args.save_model_name = f'{args.model_name}_seed{args.seed}_lr_{lr}'

            # set up logger
            logging.basicConfig(level=logging.INFO)
            logger = logging.getLogger()
            logger.setLevel(logging.DEBUG)
            os.makedirs(f"./logs_wandb/{args.model_name}/{args.dataset_name}/{args.sampling_upto}/", exist_ok=True)
            # create file handler that logs debug and higher level messages
            fh = logging.FileHandler(f"./logs_wandb/{args.model_name}/{args.dataset_name}/{args.sampling_upto}/{args.save_model_name}_{str(time.time())}.log")
            ...
            optimizer = create_optimizer(model=model, optimizer_name=args.optimizer, learning_rate=lr, weight_decay=args.weight_decay)

            model = convert_to_gpu(model, device=args.device)

            # save_model_folder = f"./saved_models/{args.model_name}/{args.dataset_name}/{args.save_model_name}/sample_{args.sampling_upto}/"
            save_model_folder = f"/scratch/{getpass.getuser()}/saved_wandb_models/{args.model_name}/{args.dataset_name}/{args.save_model_name}/"
            ...
            for epoch in range(args.num_epochs):
                ...
                wandb_log[f'train loss_{args.dataset_name}_{args.sampling_upto}'] = np.mean(train_losses)
                wandb_log[f'val loss_{args.dataset_name}_{args.sampling_upto}'] = np.mean(val_losses)
                wandb_log[f'train auc_{args.dataset_name}_{args.sampling_upto}'] = np.mean([train_metric['roc_auc'] for train_metric in train_metrics]) 
                wandb_log[f'val auc_{args.dataset_name}_{args.sampling_upto}'] = np.mean([val_metric['roc_auc'] for val_metric in val_metrics])
                wandb_log[f'train mrr_{args.dataset_name}_{args.sampling_upto}'] = np.mean([train_metric['mrr'] for train_metric in train_metrics]) 
                wandb_log[f'val mrr_{args.dataset_name}_{args.sampling_upto}'] = np.mean([val_metric['mrr'] for val_metric in val_metrics])
                wandb.log(wandb_log)
                # ==========================================================================================
            ...
            # wandb logging test
            wandb_log[f'test loss_{args.dataset_name}_{args.sampling_upto}'] = np.mean(test_losses) # f'{np.mean(test_losses):.4f}'
            wandb_log[f'test mrr_{args.dataset_name}_{args.sampling_upto}'] = test_metric_dict['mrr']
            # wandb_log['sampling_upto']=args.sampling_upto
            wandb.log(wandb_log)
    ...
    # run the sweeps
    wandb.agent(sweep_id, function=main, count=args.wnd_runs)
