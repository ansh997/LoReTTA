...
def preprocess(dataset_name: str):
    ...
    with open(dataset_name) as f:
        ...
        for idx, line in enumerate(f):
            ...
            assert ts >= previous_time, f"{idx = } and {ts = } and {previous_time = } {line = }"
            previous_time = ts
            # state_label
            label = float(e[3])
...
def preprocess_data(dataset_name: str, bipartite: bool = True, node_feat_dim: int = 172):
    ...
    # new_df = reindex(df, bipartite)  # original 
    ...
    new_df.to_csv(OUT_DF, index=False)  # edge-list
...
def check_data(dataset_name: str):
    ...
    assert origin_e_feat.shape == e_feat.shape and origin_e_feat.max() == e_feat.max() and origin_e_feat.min() == e_feat.min(), f'{origin_e_feat.shape = } and {e_feat.shape = } and {origin_e_feat.max() = } and {e_feat.max() = } and {origin_e_feat.min() = } and {e_feat.min() = }'
    # check numbers of nodes and node features
    assert origin_n_feat.shape == n_feat.shape and origin_n_feat.max() == n_feat.max() and origin_n_feat.min() == n_feat.min(), f'{origin_n_feat.shape = } and {n_feat.shape = } and {origin_n_feat.max() = } and {n_feat.max() = } and {origin_n_feat.min() = } and {n_feat.min() = }'
