...
def EL_sparsify(
    ...
    knowledge_split = 1 - 2*val_test_split
    
    save_dir = f'{save_location}/sparsified_data/{dataset_name}/{strategy}/split_{knowledge_split}'
    os.makedirs(save_dir, exist_ok=True)
    ...
    if strategy == 'random':
        ...
        # ===========================     original below this   ===========================
            ...
            if strategy == 'ts_tpr_remove_mss':
                ...
                ter_dict = calculate_temporal_edge_rank(tmp_graph, dataset_name=dataset_name, k_split=knowledge_split)
                ...
                shifts = compute_mean_shifts_with_metrics(tmp_graph, metric=metric, dataset_name=dataset_name)
...
def EL_sparsify_data(dataset_name='wikipedia', strategy='random',
    ...
    knowledge_split = 1 - 2*val_ratio
    new_save_dir = f'{save_location}/sparsified_data/{dataset_name}/{strategy}/split_{knowledge_split}'
    os.makedirs(new_save_dir, exist_ok=True)
    
    OUT_DF = f'{new_save_dir}/{dataset_name}_{strategy}_sparsified_{upto}.csv'
    
    OUT_FEAT = f'{new_save_dir}/ml_{dataset_name}.npy'
    OUT_NODE_FEAT = f'{new_save_dir}/ml_{dataset_name}_node.npy'
    ...
    EL_graph, EL_edge_raw_features, already_sparsified = EL_sparsify(_graph, edge_raw_features, strategy=strategy,
                                                                    upto=upto, dataset_name=dataset_name, selection=selection, val_test_split=val_ratio)
