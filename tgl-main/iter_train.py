...
def append_row_to_csv(filename, dataset, model, attack, ptb_rate, AP, MRR, aps, mrrs):
    fieldnames = ['dataset', 'attack', 'model', 'ptb_rate', 'AP', 'MRR', 'all_aps', 'all_mrrs']
    ...
    with open(filename, 'a', newline='') as csvfile:
        ...
        writer.writerow({
            ...
            'attack': attack,
            'model': model,
            'ptb_rate': ptb_rate if attack!='none' else None,
            'AP': AP,
            'MRR': MRR,
            'all_aps': aps,
            'all_mrrs': mrrs
...
def main(args):
    if args.attack.lower() == "none":
        print('Loading non-attacked data...')
        ...
        node_feats, edge_feats = load_attacked_feat(args.data, args.rand_edge_features, args.rand_node_features, args)
        g, df = load_attacked_graph(args.data, args)
    
        
    sample_param, memory_param, gnn_param, train_param = parse_config(args.config)
    train_edge_end = df[df['ext_roll'].gt(0)].index[0]  # not in T-Spear-Shield  
    val_edge_end = df[df['ext_roll'].gt(1)].index[0]  # not in T-Spear-Shield
        ...
        for _, rows in df[:train_edge_end].groupby(group_indexes[random.randint(0, len(group_indexes) - 1)]):
            ...
            if mailbox is not None:
                ...
                try:
                    ...
                    print(_)
                    print(f"eid: {eid}, eid.shape: {eid.shape}, eid.min: {eid.min()}, eid.max: {eid.max()}")
                    print(eid.shape), print(edge_feats.shape)
                    raise ValueError(e)
                block = None
                if memory_param['deliver_to'] == 'neighbors':
                    block = to_dgl_blocks(ret, sample_param['history'], reverse=True)[0][0]
    ...
    if args.eval_neg_samples > 1:
        print('\ttest AP:{:2%}  test MRR:{:2%}'.format(ap, auc))
    else:
        print('\ttest AP:{:2%}  test AUC:{:2%}'.format(ap, auc))
    return ap, auc
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_iter', type=int, default=1, help='number of iterations to run the script')
    parser.add_argument('--id', type=str, default=None, help='to id the run.')
    parser.add_argument("--attack", type=str, default="none", choices=["none", "random", "proposed", "degree", "pagerank", "preference", "jaccard"])
    args=parser.parse_args()
    
    # python iter_train.py --data=WIKI --config=configs/TGN.yaml --gpu=0 --eval_neg_samples=100 --num_iter=5 --id=test --ptb_rate=0.3
    ...
    for seed in range(args.num_iter):
        print('*'*20, f'Iteration {seed}', '*'*20)
        set_seed(seed)

        ap, mrr = main(args=args)

        aps.append(ap)
        mrrs.append(mrr)
    
    mean_ap, std_ap = np.mean(aps), np.std(aps)
    mean_mrr, std_mrr = np.mean(mrrs), np.std(mrr)

    print(f"AP: {mean_ap:.2%} ± {std_ap:.2%}")
    print(f"MRR: {mean_mrr:.2%} ± {std_mrr:.2%}")

    current_time = datetime.now().strftime('%Y%m%d')

    args.id = current_time if args.id is None else args.id 

    filename = f'{args.data}_results_neg_samples{args.eval_neg_samples}_{args.id}.csv'

    append_row_to_csv(filename, args.data, f'{args.config.split("/")[-1].split(".")[0]}', args.attack, args.ptb_rate, f"{mean_ap:.2%} ± {std_ap:.2%}", f"{mean_mrr:.2%} ± {std_mrr:.2%}", aps, mrrs)
    print('*'*20, 'Results saved to', filename, '*'*20)