...
def append_row_to_csv(filename, dataset, ss, upto, AP, MRR):
    # Define the fieldnames (column headers)
    # fieldnames = ['dataset', 'nss', 'ss', 'upto', 'AP', 'MRR']
    fieldnames = ['dataset', 'ss', 'upto', 'AP', 'MRR']
...
# parser.add_argument('--nss', type=str, help='negative sampling strategy name', default='random')
...
# g, df = load_poisoned_graph(args.data, args.nss, args.ss, args.upto)
g, df = load_poisoned_graph(args.data, '', args.ss, args.upto, flag=False)

# print(len(edge_feats), len(df))
# print(type(edge_feats), type(df))
# exit()
...
filename = f'{args.data}_{args.config.split("/")[-1].split(".")[0]}_results_neg_samples{args.eval_neg_samples}_{current_time}.csv'
if args.eval_neg_samples > 1:
    print('\ttest AP:{:4f}  test MRR:{:4f}'.format(ap, auc))
    append_row_to_csv(filename, args.data, args.ss, args.upto, ap, auc)
else:
    print('\ttest AP:{:4f}  test AUC:{:4f}'.format(ap, auc))
    append_row_to_csv(filename, args.data, args.nss, args.ss, args.upto, ap, auc)
