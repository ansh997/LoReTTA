...
load_location = f'{scratch_location}/poisoned_data'
...
# have to convert each np array to object type
np.savez(f'{save_location}/modified_{args.data}_{args.ss}_sparsified_{args.upto}_ext_full.npz', 
         indptr=np.array(ext_full_indptr, dtype=object), 
         indices=np.array(ext_full_indices, dtype=object), 
         ts=np.array(ext_full_ts, dtype=object), 
         eid=np.array(ext_full_eid, dtype=object))
