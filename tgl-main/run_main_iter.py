...
def append_row_to_csv(filename, dataset, model, ss, upto, AP, MRR, aps, mrrs):
    # Define the fieldnames (column headers)
    # fieldnames = ['dataset', 'nss', 'ss', 'upto', 'AP', 'MRR']
    fieldnames = ['dataset', 'Model', 'ss', 'upto', 'AP', 'MRR', 'APs', 'MRRs']
...
def main(args):
    node_feats, edge_feats = load_poisoned_feat(args.data, args.rand_edge_features,
                                            args.rand_node_features, args.ss, args.upto)

    g, df = load_poisoned_graph(args.data, '', args.ss, args.upto, flag=False)
...
if __name__ == "__main__":
    ...
    filename = f'{args.data}_Ours_results_neg_samples{args.eval_neg_samples}_{args.id}.csv'

    append_row_to_csv(filename, args.data, f'{args.config.split("/")[-1].split(".")[0]}', args.ss, args.upto, f"{mean_ap:.4f} ± {std_ap:.4f}", f"{mean_mrr:.4f} ± {std_mrr:.4f}", aps, mrrs)
