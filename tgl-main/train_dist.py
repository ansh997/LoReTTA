...
backend = 'gloo'
set_seed(args.seed)
torch.distributed.init_process_group(backend=backend, timeout=datetime.timedelta(0, 3600))
nccl_group = torch.distributed.new_group(ranks=list(range(args.num_gpus)), backend=backend)
# world_size = torch.distributed.get_world_size()  # or pass from args
...
if args.local_rank > 0 and args.local_rank < args.num_gpus:
    node_feats = None
    edge_feats = None
    if os.path.exists('/raid/t2/TGN_adv/tspear/DATA/{}/node_features.pt'.format(args.data)):
        node_feats = get_shared_mem_array('node_feats', (dim_feats[0], dim_feats[1]), dtype=dim_feats[2])
    if os.path.exists('/raid/t2/TGN_adv/tspear/DATA/{}/edge_features.pt'.format(args.data)):
...
if args.local_rank < args.num_gpus:
    ...
    # data parallelization:= Distributed Data Parallel (DDP) - This ensures that each replica of the model gets synchronized gradients.
    model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[args.local_rank],
                                                    process_group=nccl_group, output_device=args.local_rank,
