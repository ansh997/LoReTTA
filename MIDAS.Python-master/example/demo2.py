...
Also computes AUPRC (AP) on stabilized scores and appends
(dataset, algorithm, auprc_ap) to out/results_new.csv.
"""
import argparse
import sys
...
# --------------------------------------------------------------------------- #
# Helper: append AUPRC(AP) to results_new.csv                                 #
# --------------------------------------------------------------------------- #
def append_auprc_to_results_new(out_dir: Path, dataset: str, algorithm: str, auprc_ap: float):
    out_path = out_dir / "results_new.csv"
    write_header = not out_path.exists()
    with out_path.open("a", newline="") as fh:
        writer = csv.DictWriter(fh, fieldnames=["dataset", "algorithm", "auprc_ap"])
        if write_header:
            writer.writeheader()
        writer.writerow({
            "dataset": dataset,
            "algorithm": algorithm,
            "auprc_ap": f"{auprc_ap:.6f}" if not np.isnan(auprc_ap) else ""
        })
...
if __name__ == "__main__":
    ...
    algorithm = f"midas_{midas.__class__.__name__.lower()}"
    ...
    # 2. Convert to binary via Youden-J threshold (on stabilized scores)
    scores = np.log1p(np.nan_to_num(scores_raw, nan=0.0, posinf=1e10))

    # --- AUPRC (AP) threshold-free summary on stabilized scores ----------
    if labels.min() != labels.max():
        auprc_ap = average_precision_score(labels, scores)
        print(f"AUPRC (AP)         : {auprc_ap:.4f}")
    else:
        auprc_ap = np.nan
        print("AUPRC (AP)         : n/a")
    ...
    # 5. Append AUPRC (AP) to results_new.csv (dataset, algorithm, auprc_ap)
    append_auprc_to_results_new(out_dir, args.data, algorithm, auprc_ap)
    print(f"AUPRC (AP) appended to {out_dir / 'results_new.csv'}")
